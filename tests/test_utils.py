...
class TestUtils(unittest.TestCase):
    ...
    def test_seq_data_loading_pipeline(self):
        with initialize():
            ...
            assert len(x) == 3
            assert x[0].shape == (2, 29, soc_data.STATE_SIZE + soc_data.ACTION_SIZE, 7, 7)
            assert x[1].shape == (2, 29, soc_data.STATE_SIZE, 7, 7)
            assert x[2].shape == (2, 29, soc_data.STATE_SIZE, 7, 7)
